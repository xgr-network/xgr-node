...
var (
	...
	ErrMissingChainIDConfig    = errors.New("missing txpool chain id configuration")
)
...
func (p *TxPool) validateTx(tx *types.Transaction) error {
	...
	if tx.IsContractCreation() && forks.EIP3860 && len(tx.Input) > state.TxPoolMaxInitCodeSize {
	...
	if tx.Type == types.AccessListTx {
		if !forks.EIP2930 {
			...
			return fmt.Errorf("%w: type %d rejected, EIP-2930 hardfork is not enabled", ErrTxTypeNotSupported, tx.Type)
		}

		blockNumber, err := forkmanager.GetInstance().GetForkBlock(chain.TxHashWithType)
		if err != nil {
			metrics.IncrCounter([]string{txPoolMetrics, "access_list_not_allowed"}, 1)

			return fmt.Errorf("%w: access list txs are not allowed when txHashWithType is not active", ErrTxTypeNotSupported)
		}

		if blockNumber > currentBlockNumber {
			metrics.IncrCounter([]string{txPoolMetrics, "access_list_not_allowed"}, 1)

			return fmt.Errorf("%w: access list txs are not allowed before txHashWithType at %d", ErrTxTypeNotSupported, blockNumber)
		}

		if tx.GasPrice == nil {
			metrics.IncrCounter([]string{txPoolMetrics, "access_list_missing_gas_price"}, 1)

			return errors.New("gas price is not set")
		}
	}
	...
	if tx.Type == types.DynamicFeeTx {
		...
		if err != nil {
			...
			return fmt.Errorf("%w: dynamic fee txs are not allowed when txHashWithType is not active", ErrTxTypeNotSupported)
		}
	...
	intrinsicGas, err := state.TransactionGasCost(
		tx,
		forks.Homestead,
		forks.Istanbul,
		forks.EIP3860,
...
func (p *TxPool) addTx(origin txOrigin, tx *types.Transaction) error {
	...
	// Ensure ChainID is set (and consistent) before validation/signature recovery.
	// Typed txs MUST carry chainId in the payload (EIP-2718 / EIP-2930 / EIP-1559).
	if tx.Type == types.DynamicFeeTx || tx.Type == types.AccessListTx {
		if p.chainID == nil {
			return ErrMissingChainIDConfig
		}

		if tx.ChainID == nil {
			tx.ChainID = new(big.Int).Set(p.chainID)
		} else if tx.ChainID.Cmp(p.chainID) != 0 {
			metrics.IncrCounter([]string{txPoolMetrics, "invalid_chain_id"}, 1)
			return fmt.Errorf("invalid chain id: got %s want %s", tx.ChainID.String(), p.chainID.String())
		}
	}
