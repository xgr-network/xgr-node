...
import (
	...
	"github.com/xgr-network/xgr-node/helper/tests"
...
func TestAddTxErrors(t *testing.T) {
	...
	t.Run("ErrMissingChainIDConfig for typed tx when pool chain id is nil", func(t *testing.T) {
		t.Parallel()
		...
		assert.ErrorIs(t, err, ErrMissingChainIDConfig)
	})

	t.Run("ErrMissingChainIDConfig for access list tx when pool chain id is nil", func(t *testing.T) {
		t.Parallel()
...
func Test_TxPool_validateTx(t *testing.T) {
	forkManagerMu.Lock()
	defer forkManagerMu.Unlock()

	fm := forkmanager.GetInstance()
	fm.Clear()
	fm.RegisterFork(chain.TxHashWithType, nil)
	require.NoError(t, fm.ActivateFork(chain.TxHashWithType, 0))
	defer fm.Clear()
	...
	signTx := func(transaction *types.Transaction) *types.Transaction {
		if transaction.Type == types.DynamicFeeTx || transaction.Type == types.AccessListTx {
			transaction.ChainID = big.NewInt(100)
		}
	...
	t.Run("access list tx is rejected when txHashWithType fork is not active", func(t *testing.T) {
		fm.Clear()
		fm.RegisterFork(chain.TxHashWithType, nil)
		defer func() {
			fm.Clear()
	...
	t.Run("dynamic fee tx is rejected when txHashWithType fork is not active", func(t *testing.T) {
		fm.Clear()
		fm.RegisterFork(chain.TxHashWithType, nil)
		defer func() {
			fm.Clear()
