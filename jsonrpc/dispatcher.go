...
import (
	...
	"github.com/xgr-network/xgr-node/engineadapter/stub"
	"github.com/xgr-network/xgr-node/engineiface"
...
func (d *Dispatcher) registerEndpoints(store JSONRPCStore) error {
	...
	if engineMode == engineiface.ModeEmbedded {
		if !xgrsvc.EmbeddedAvailable() {
			return fmt.Errorf("engine.mode=embedded but embedded xgr endpoint package is not linked")
		}

		engineEOA := strings.ToLower(strings.TrimSpace(os.Getenv("ENGINE_EOA")))
		var enginePub33 []byte
		if eoa, pub, err := xgrsvc.LoadEngineConfigFlex(nil); err == nil {
			if strings.TrimSpace(eoa) != "" {
				engineEOA = strings.ToLower(strings.TrimSpace(eoa))
			}
			if len(pub) > 0 {
				enginePub33 = append([]byte(nil), pub...)
			}
	...
	if err = d.registerService("xgr", d.endpoints.XGR); err != nil {
		return err
	}
...
func (d *Dispatcher) registerService(serviceName string, service interface{}) error {
	...
	for i := 0; i < st.NumMethod(); i++ {
		...
		fmt.Println("Registering:", serviceName+"_"+name)
