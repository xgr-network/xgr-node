...
func TestEth_GasPrice_WithLondonFork(t *testing.T) {
	...
	store.blocks = []*types.Block{
		{
			Header: &types.Header{Number: uint64(1), BaseFee: baseFee},
		},
	}
	...
	t.Run("returns latest header base fee", func(t *testing.T) {
		res, err := eth.GasPrice()

		assert.NoError(t, err)
		assert.NotNil(t, res)
		assert.Equal(t, argUint64(baseFee), res)
	})
}
...
func TestEth_GasPrice_WithoutLondonFork(t *testing.T) {
	...
	eth := newTestEthEndpointWithPriceLimit(store, 0)

	t.Run("returns latest header base fee", func(t *testing.T) {
		store.averageGasPrice = baseFee - 100
