...
// satAddU64 returns a+b with uint64 saturation (clamps to MaxUint64 on overflow).
...
// satMulU64 returns a*b with uint64 saturation (clamps to MaxUint64 on overflow).
...
func (e *Eth) suggestFees() (uint64, uint64, uint64, uint64, error) {
	header := e.store.Header()
	baseFee := header.BaseFee
	tip := uint64(0)
	gasPrice := baseFee
	feeCap := satMulU64(baseFee, 2)

	return baseFee, tip, feeCap, gasPrice, nil
}

func (e *Eth) getGasPrice() (uint64, error) {
	_, _, _, gasPrice, err := e.suggestFees()
	if err != nil {
		return 0, err
	}
...
func (e *Eth) fillTransactionGasPrice(tx *types.Transaction) error {
	// Dynamic fee transactions need both tip cap and fee cap for broad tooling compatibility.
	if tx.Type == types.DynamicFeeTx {
		if tx.GasTipCap != nil && tx.GasFeeCap != nil {
			return nil
		}

		_, tip, feeCap, _, err := e.suggestFees()
		if err != nil {
			return err
		}

		if tx.GasTipCap == nil {
			tx.GasTipCap = new(big.Int).SetUint64(tip)
		}
		if tx.GasFeeCap == nil {
			tx.GasFeeCap = new(big.Int).SetUint64(feeCap)
		}
...
func (e *Eth) EstimateGas(arg *txnArgs, rawNum *BlockNumber) (interface{}, error) {
	...
	// IMPORTANT:
	// A tx with empty data can still execute contract code (fallback/receive) if `to` is a contract.
	// Returning only intrinsic gas (e.g. 21000) underestimates the real requirement in that case.
	isContractRecipient := func() (bool, error) {
		...
		to := transaction.To
		if to == nil || *to == types.ZeroAddress {
			return false, nil
		}

		// Prefer querying code directly (robust across Account struct variants)
		code, err := e.store.GetCode(header.StateRoot, *to)
		if err != nil {
			if errors.Is(err, ErrStateNotFound) {
				// Not initialized => no code
				return false, nil
			}
			return false, err
		}
	...
	if transaction.IsValueTransfer() {
		// Only short-circuit if recipient is NOT a contract (EOA transfer).
		isContract, err := isContractRecipient()
		if err != nil {
			return nil, err
		}
		if isContract {
			// Must execute via ApplyTxn + binary search to include fallback/receive gas.
			goto ESTIMATE_BY_EXECUTION
		}
		...
		gasCost, err := state.TransactionGasCost(
			transaction,
			forksInTime.Homestead,
			forksInTime.Istanbul,
			forksInTime.EIP3860,
			forksInTime.EIP2930,
		)
		if err != nil {
...
func (e *Eth) MaxPriorityFeePerGas() (interface{}, error) {
	_, tip, _, _, err := e.suggestFees()
	if err != nil {
		return nil, err
	}

	return argBigPtr(new(big.Int).SetUint64(tip)), nil
}
