...
// rpcAccessTuple is the JSON-RPC representation of an EIP-2930 access-list entry.
// Keep this here to avoid changing core types (types.AccessTuple), which can break persistence / internals.
type rpcAccessTuple struct {
	Address     types.Address `json:"address"`
	StorageKeys []types.Hash  `json:"storageKeys"`
}

type rpcAccessList []rpcAccessTuple
...
type transaction struct {
	...
	AccessList  *rpcAccessList `json:"accessList,omitempty"`
	V           argBig         `json:"v"`
	R           argBig         `json:"r"`
	S           argBig         `json:"s"`
...
func toTransaction(
	...
	if t.Type == types.AccessListTx || t.Type == types.DynamicFeeTx {
		// Always return [] (not null) for typed txs
		mapped := make(rpcAccessList, 0, len(t.AccessList))
		for _, tup := range t.AccessList {
			mapped = append(mapped, rpcAccessTuple{
				Address:     tup.Address,
				StorageKeys: tup.StorageKeys,
			})
		}
		res.AccessList = &mapped
	}
	if txIndex != nil {
