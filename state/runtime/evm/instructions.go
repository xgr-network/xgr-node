...
const (
	...
	sstoreResetGasEIP2929 uint64 = 5000 - coldSloadCost // 2900
)

func (c *state) accessList() *runtime.AccessList {
	if !c.config.EIP2929 {
		return nil
	}
	return c.host.GetTxContext().AccessList
}

func (c *state) accessAddressGas(addr types.Address) uint64 {
	al := c.accessList()
	if al == nil {
		return 0
	}
...
func (c *state) accessStorageGas(addr types.Address, slot types.Hash) uint64 {
	al := c.accessList()
	if al == nil {
		return 0
	}
...
func opSload(c *state) {
	...
	var gas uint64
	if c.config.EIP2929 {
		gas = c.accessStorageGas(c.msg.Address, key)
	...
	val := c.host.GetStorage(c.msg.Address, key)
...
func opSStore(c *state) {
	...
	// EIP-2929: first access to a (addr,slot) in this frame costs coldSloadCost
	if c.config.EIP2929 {
		if al := c.accessList(); al != nil && !al.ContainsSlot(c.msg.Address, key) {
			al.AddSlot(c.msg.Address, key)
			if !c.consumeGas(coldSloadCost) {
				return
			}
		}
	}
...
func opExtCodeHash(c *state) {
	...
	var gas uint64
	if c.config.EIP2929 {
		gas = c.accessAddressGas(address)
	} else if c.config.Istanbul {
		gas = 700
	} else {
		gas = 400
...
func opSelfDestruct(c *state) {
	...
	if c.config.EIP150 {
		...
		// EIP-2929: cold beneficiary costs extra
		if c.config.EIP2929 {
			if al := c.accessList(); al != nil && !al.ContainsAddress(address) {
...
func opCreate(op OpCode) instruction {
	return func(c *state) {
		...
		// EIP-2929: created address becomes warm immediately
		if c.config.EIP2929 {
			if al := c.host.GetTxContext().AccessList; al != nil {
				al.AddAddress(contract.Address)
			}
		}
...
func (c *state) buildCallContract(op OpCode) (*runtime.Contract, uint64, uint64, error) {
	...
	var gasCost uint64
	if c.config.EIP2929 {
		gasCost = c.accessAddressGas(addr)
	} else if c.config.EIP150 {
		gasCost = 700
	} else {
		gasCost = 40
...
func (c *state) buildCreateContract(op OpCode) (*runtime.Contract, error) {
	...
	if c.config.EIP3860 {
		if uint64(len(input)) > maxInitCodeSize {
			c.exit(errOutOfGas)
		...
		hi, cost := bits.Mul64(words, initCodeWordGas)
		if hi != 0 {
			c.exit(errGasUintOverflow)
