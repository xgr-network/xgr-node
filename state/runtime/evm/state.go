...
type stackFrame struct {
	pc     uint64
	locals map[string]*big.Int // oder []uint256.Int oder []big.Int
	// ggf. weitere Felder wie memory snapshot, gas etc.
}
...
type state struct {
	...
	returnData       []byte
	ret              []byte
	transientStorage map[string]*big.Int
	pc               uint64
...
func (c *state) Run() ([]byte, error) {
	...
	for !c.stop {
		...
		//		hexOp := fmt.Sprintf("0x%02x", uint8(op))
		//		fmt.Println("Opcode in hex:", hexOp)
