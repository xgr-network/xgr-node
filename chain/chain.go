...
func importChain(content []byte) (*Chain, error) {
	...
	// XGR: apply genesis-driven registry configuration (consensus-critical).
	// This avoids any post-deploy code patching and keeps dev/test/main deterministic per genesis.json.
	if chain.Params != nil {
		if chain.Params.EngineRegistryAddress != (types.Address{}) {
			EngineRegistryAddress = chain.Params.EngineRegistryAddress
		}
		if chain.Params.BootstrapEngineEOA != (types.Address{}) {
			BootstrapEngineEOA = chain.Params.BootstrapEngineEOA
		}
	}
