...
func unmarshalAccessListFrom(v *fastrlp.Value) (AccessList, error) {
	...
	accessList := make(AccessList, 0, len(elems))
	for _, elem := range elems {
		...
		if len(tuple) != 2 {
			return nil, fmt.Errorf("access tuple must have 2 elements, got %d", len(tuple))
		}

		addrBytes, err := tuple[0].Bytes()
		if err != nil {
			return nil, err
		}

		if len(addrBytes) != AddressLength {
			return nil, fmt.Errorf("invalid access-list address length %d", len(addrBytes))
		}

		keysElems, err := tuple[1].GetElems()
		if err != nil {
			return nil, err
		}

		keys := make([]Hash, 0, len(keysElems))
		for _, keyElem := range keysElems {
			...
			if len(keyBytes) != HashLength {
				return nil, fmt.Errorf("invalid access-list storage key length %d", len(keyBytes))
			}

			keys = append(keys, BytesToHash(keyBytes))
		}

		accessList = append(accessList, AccessTuple{
			Address:     BytesToAddress(addrBytes),
			StorageKeys: keys,
		})
	}
...
func (t *Transaction) unmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// access list
	if t.Type == DynamicFeeTx || t.Type == AccessListTx {
		if t.AccessList, err = unmarshalAccessListFrom(getElem()); err != nil {
			return err
		}
	}
