...
// MarshalAccessListWith serializes an EIP-2930 access list into an RLP value using the provided arena.
// Exported for reuse in other packages (e.g. signer hashing).
func MarshalAccessListWith(arena *fastrlp.Arena, accessList AccessList) *fastrlp.Value {
	list := arena.NewArray()

	for _, tuple := range accessList {
		entry := arena.NewArray()
		entry.Set(arena.NewCopyBytes(tuple.Address.Bytes()))

		keys := arena.NewArray()
		for _, key := range tuple.StorageKeys {
			keys.Set(arena.NewCopyBytes(key.Bytes()))
		}
...
func (t *Transaction) MarshalRLPWith(arena *fastrlp.Arena) *fastrlp.Value {
	...
	// Check Transaction1559Payload there https://eips.ethereum.org/EIPS/eip-1559#specification
	if t.Type == DynamicFeeTx || t.Type == AccessListTx {
		vv.Set(MarshalAccessListWith(arena, t.AccessList))
	}
