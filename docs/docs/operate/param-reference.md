...
| `--data-dir string`   | The directory for the XGRChain data if the local FS is used   | ""            | YES       |
...
- `--config` and `--data-dir`: These flags are mutually exclusive. Use either `--config` to specify the path to the SecretsManager config file or `--data-dir` to set the directory for the XGRChain data if the local FS is used.
...
| `--name` | The name for the chain | "xgrchain" | NO | `genesis --name "test-chain"` | NO |
...
| `--data-dir` string | The data directory used for storing XGRChain client data. | “” | YES | Command: server Flag:--data-dir “./test-chain-1” | NO |
