...
The `xgrchain` rootchain server command starts an ethereum/client-go container, which runs a new Geth node.
...
You should see output similar to the following, indicating that the rootchain server is now running:
  ...
  ```bash
  {"status":"Pulling from xgrnetwork/go-ethereum-console","id":"latest"}
  {"status":"Digest: sha256:6aad124b6775b96d05c94850dcafde45911f7bb2b473328dc4a792b1ffb2bdb6"}
  {"status":"Status: Image is up to date for ghcr.io/xgrnetwork/go-ethereum-console:latest"}
...
| `--data-dir`                 | The directory for the XGRChain data if the local FS is used.              | `--data-dir test-chain-`                     |
...
To deploy the rootchain contracts, we use the `xgrchain rootchain deploy` command. Before deployment, consider the following actions:
...
| `--data-dir`    | Directory for the XGRChain data if the local FS is used                                            |         |
...
To fund the validators' accounts on the rootchain, we use the `xgrchain rootchain fund` command. When executed with the appropriate flags, it will:
...
Here's an example of how to fund a validator account (replace address value with your own):
  ...
  ```bash
  ./xgrchain rootchain fund --addresses 0x77C1eedFf656477462ce16084fE5Dc7F8a2507B9 --amounts 1000000000000000000
  ```
