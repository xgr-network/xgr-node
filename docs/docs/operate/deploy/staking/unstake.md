...
First, check your validator information by running the `xgrchain polybft validator-info` command.
...
| `--data-dir` | The directory for the XGRChain data if the local FS is used | `/path/to/data/dir` |
...
```bash
./xgrchain polybft validator-info --data-dir ./test-chain-1
```
...
To unstake, use the `xgrchain polybft unstake` command.
...
| --data-dir          | Directory for the XGRChain data if the local FS is used | --data-dir /path/to/data/dir          |
| --jsonrpc           | JSON-RPC interface                                         | --jsonrpc 0.0.0.0:8545                |

</details>

```bash
./xgrchain polybft unstake \
...
Once you have successfully unstaked, you will need to withdraw your unstaked tokens from the Edge-powered chain to the rootchain. To do so, use the `xgrchain polybft withdraw-child` command.
...
| `--data-dir` | The directory for the XGRChain data if the local FS is used | `~/xgrchain/data` |
| `--jsonrpc` | The JSON-RPC interface (default "0.0.0.0:8545") | `127.0.0.1:8545` |

</details>

```bash
./xgrchain polybft withdraw-child \
    --account-dir <DATA_DIR> \
    --jsonrpc <JSONRPC_ADDR>
```
...
Once the exit event has been included in a checkpoint, you can send an exit transaction to execute the transaction on the rootchain. To do so, use the `xgrchain bridge exit` command.
...
Finally, you can withdraw your tokens from the rootchain to your address by using the `xgrchain polybft withdraw-root` command.
...
| --data-dir             | directory for the XGRChain data     | --data-dir /path/to/data/dir       |
| --jsonrpc              | JSON-RPC interface                       | --jsonrpc 0.0.0.0:8545             |
...
| --to                   | address where to withdraw                | --to 0x456def                      |
  ...
  ```bash
  ./xgrchain polybft withdraw-root \
    --account-dir <DATA_DIR> \
