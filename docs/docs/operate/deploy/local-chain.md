...
The `xgrchain polybft-secrets` command is used to generate account secrets for validators. The command initializes private keys for the consensus client (validators + networking) to a Secrets Manager config file.
...
| `--data-dir`    | The directory for the XGRChain data if the local FS is used.                                          | `--data-dir /path/to/dir`  |
...
Next, create a config.json file within your xgrchain directory with the following contents:
...
> The secrets output can be retrieved again if needed by running the following command: `./xgrchain polybft-secrets --data-dir test-chain-X/`
