...
Bootnodes act as rendezvous servers that help new nodes discover and connect to the network. The xgrchain command allows you to specify one or more bootnodes while creating the genesis file. Bootnodes are defined using libp2p multiaddrs, which contain information about the protocol, network address, and node port number.
