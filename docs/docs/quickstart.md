...
To access the pre-built releases, visit the [GitHub releases page](https://github.com/xgr-network/xgr-node/releases). The client provides cross-compiled AMD64/ARM64 binaries for Darwin and Linux.
...
function initPolybftConsensus() {
  ...
  address1=$(./xgrchain polybft-secrets --insecure --data-dir test-chain-1 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address2=$(./xgrchain polybft-secrets --insecure --data-dir test-chain-2 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address3=$(./xgrchain polybft-secrets --insecure --data-dir test-chain-3 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address4=$(./xgrchain polybft-secrets --insecure --data-dir test-chain-4 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
}
...
function createGenesis() {
  ./xgrchain genesis $genesis_params \
    ...
    --native-token-config "XGR:XGR:18:true:$address1" \
...
function initRootchain() {
  ...
  if [ "$1" == "write-logs" ]; then
    ...
    ./xgrchain rootchain server 2>&1 | tee ./rootchain-server.log &
  else
    ./xgrchain rootchain server >/dev/null &
  fi
  ...
  ./xgrchain polybft stake-manager-deploy \
  ...
  ./xgrchain polybft whitelist-validators \
    --addresses ${address1},${address2},${address3},${address4} \
  ...
  while [ $counter -le 4 ]; do
    ...
    ./xgrchain polybft register-validator \
      --supernet-manager ${customSupernetManagerAddr} \
      --data-dir test-chain-${counter} \
      --jsonrpc http://127.0.0.1:8545

    ./xgrchain polybft stake \
      --data-dir test-chain-${counter} \
...
function startServerFromBinary() {
  if [ "$1" == "write-logs" ]; then
    ...
    ./xgrchain server --data-dir ./test-chain-1 --chain genesis.json \
