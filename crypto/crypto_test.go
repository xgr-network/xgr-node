...
func TestBLSEIP2537Conversions(t *testing.T) {
	sk, err := GenerateBLSKey()
	require.NoError(t, err)

	compressedPK, err := BLSSecretKeyToPubkeyBytes(sk)
	require.NoError(t, err)
	eipPK, err := BLSPublicKeyToEIP2537(compressedPK)
	require.NoError(t, err)
	require.Len(t, eipPK, 128)
	require.Equal(t, make([]byte, 16), eipPK[:16])
	require.Equal(t, make([]byte, 16), eipPK[64:80])

	var pkBlob [bls_sig.PublicKeySize]byte
	copy(pkBlob[:], compressedPK)
	pkPoint, err := new(bls12381.G1).FromCompressed(&pkBlob)
	require.NoError(t, err)
	pkRaw := pkPoint.ToUncompressed()
	require.Equal(t, pkRaw[:48], eipPK[16:64])
	require.Equal(t, pkRaw[48:], eipPK[80:128])

	sigCompressed, err := SignByBLS(sk, []byte("xgr-eip2537-vector"))
	require.NoError(t, err)
	eipSig, err := BLSSignatureToEIP2537(sigCompressed)
	require.NoError(t, err)
	require.Len(t, eipSig, 256)

	var sigBlob [bls_sig.SignatureSize]byte
	copy(sigBlob[:], sigCompressed)
	sigPoint, err := new(bls12381.G2).FromCompressed(&sigBlob)
	require.NoError(t, err)
	sigRaw := sigPoint.ToUncompressed()
	require.Equal(t, sigRaw[48:96], eipSig[16:64])
	require.Equal(t, sigRaw[:48], eipSig[80:128])
	require.Equal(t, sigRaw[144:192], eipSig[144:192])
	require.Equal(t, sigRaw[96:144], eipSig[208:256])
}


func TestInterchainEIP2537Vector(t *testing.T) {
	sk, err := new(bls_sig.SecretKey).Generate(make([]byte, 32))
	require.NoError(t, err)

	msg := []byte("xgr-eip2537-vector")
	pk, err := BLSSecretKeyToPubkeyBytes(sk)
	require.NoError(t, err)
	sig, err := SignByBLS(sk, msg)
	require.NoError(t, err)
	eipPK, err := BLSPublicKeyToEIP2537(pk)
	require.NoError(t, err)
	eipSig, err := BLSSignatureToEIP2537(sig)
	require.NoError(t, err)

	require.Equal(t,
		"a695ad325dfc7e1191fbc9f186f58eff42a634029731b18380ff89bf42c464a42cb8ca55b200f051f57f1e1893c68759",
		strings.TrimPrefix(hex.EncodeToHex(pk), "0x"),
	)
	require.Equal(t,
		"89db6117270d4bc0494417ec453aac0d2646dc40309c71d27babd5c0018ff829b00d1edb3dd2ae6dfa6f4e6e0d27720d17088961632badc833b832b51fe006ae532b7238672ca5fc446e3b2b80fe1b2be767eb7b765698ae722b64cefc96e685",
		strings.TrimPrefix(hex.EncodeToHex(sig), "0x"),
	)
	require.Equal(t,
		"",
		strings.TrimPrefix(hex.EncodeToHex(eipPK), "0x"),
	)
	require.Equal(t,
		"",
		strings.TrimPrefix(hex.EncodeToHex(eipSig), "0x"),
	)
}
