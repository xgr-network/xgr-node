...
func TestJsonRPC(t *testing.T) {
	...
	t.Run("eth_getBalance", func(t *testing.T) {
		...
		balance1, err = client.GetBalance(key1.Address(), ethgo.Latest)
		require.NoError(t, err)
		require.Zero(t, balance1.Cmp(newBalance))
		...
		require.NoError(t, err)
		txPrice := new(big.Int).Mul(
			new(big.Int).SetUint64(framework.TestGasPriceUint64()),
		...
		amountToSend := new(big.Int).Sub(newBalance, txPrice)
	...
	t.Run("eth_getTransactionCount", func(t *testing.T) {
		...
		txn, err := srv.Txn(fund).Transfer(key1.Address(), ethgo.Ether(1)).Send()
		require.NoError(t, err)
		txn.NoFail(t)
