...
import (
	...
	ibftSwitch "github.com/xgr-network/xgr-node/command/ibft/switch"
	...
	"github.com/xgr-network/xgr-node/server/proto"
	txpoolProto "github.com/xgr-network/xgr-node/txpool/proto"
...
const (
	DefaultGasPrice = 100_000_000_000 // 100 gwei floor for XGR (kept in sync with chain.MinBaseFee)
...
func resolveBinary() string {
	if bin := os.Getenv("EDGE_BINARY"); bin != "" {
		return bin
	}

	if bin := os.Getenv("XGRCHAIN_BINARY"); bin != "" {
		return bin
	}

	if _, err := exec.LookPath("xgrchain"); err == nil {
		return "xgrchain"
	}
